...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Author foxthefox';
    var email     = grunt.option('email')  || 'foxthefox@mail.com';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new hkv name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/hkv.png')) {
            fs.renameSync(__dirname + '/admin/hkv.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/hkv.html')) {
            fs.renameSync(__dirname + '/widgets/hkv.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/hkv/js/hkv.js')) {
            fs.renameSync(__dirname + '/widgets/hkv/js/hkv.js', __dirname + '/widgets/hkv/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/hkv')) {
            fs.renameSync(__dirname + '/widgets/hkv',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Hkv/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Hkv'
                        },
                        {
                            match: /Author foxthefox/g,
                            replacement: author
                        },
                        {
                            match: /foxthefox@mail.com/g,
                            replacement: email
                        }
                    ]
                },
